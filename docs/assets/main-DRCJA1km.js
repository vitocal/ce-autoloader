import{_ as l,C as w}from"./index-Cm2yFaKC.js";import{x as v,i as x,a as k}from"./lit-element-OgMYHPKI.js";class y extends HTMLElement{constructor(){super(),this.error="",this.stack="",this.error=this.getAttribute("error")||"",this.stack=this.getAttribute("stack")||""}static get observedAttributes(){return["error","stack"]}attributeChangedCallback(t,r,s){t==="error"?(this.error=s,this.render()):t==="stack"&&(this.stack=s,this.render())}connectedCallback(){this.attachShadow({mode:"open"}),this.render()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`<style>
        :host {
          display: block;
      ...
      </details>`)}}customElements.define("error-fallback",y);const f=Symbol.for(""),$=e=>{if(e?.r===f)return e?._$litStatic$},p=e=>({_$litStatic$:e,r:f}),g=new Map,E=e=>(t,...r)=>{const s=r.length;let i,o;const a=[],h=[];let c,n=0,u=!1;for(;n<s;){for(c=t[n];n<s&&(o=r[n],(i=$(o))!==void 0);)c+=i+t[++n],u=!0;n!==s&&h.push(o),a.push(c),n++}if(n===s&&a.push(t[s]),u){const m=a.join("$$lit$$");(t=g.get(m))===void 0&&(a.raw=a,g.set(m,t=a)),r=h}return e(t,...r)},d=E(v);class _ extends x{static styles=k`
    @layer component {
		hero-example {
			display: grid;
